	...
	func _init(spell: SpellNames):
		match spell:
			...
			SpellNames.HEALING_TOUCH:
				populate("Healing Touch", preload("res://healing_touch.tscn"), 2, -2, 1, 1, 0.3, true)
			SpellNames.FIREBALL:
				populate("Fireball", preload("res://fireball.tscn"), 3, 1, 8, 0.9, 0, false, 3)
			_:
				populate("Magic Missile", preload("res://magic_missile.tscn"), 1, 1, 6, 1, 0)	#default to magic missile todo change to rock
	
	func populate(new_name, new_node, new_cost, new_damage, new_range, new_hit_chance, new_crit_chance, new_self_cast = false, new_radius = 0):
		name = new_name
		spell_node = new_node
		cost = new_cost
		damage = new_damage
